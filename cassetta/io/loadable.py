...
from cassetta.core.typing import Union, IO, Type, Iterable
...
class LoadableMixin:
    ...
    def serialize(self) -> dict:
        ...
        """
        klass = type(self)
        base = klass.__bases__[0] if klass.__bases__ else object
        if issubclass(base, DynamicLoadableMixin):
    ...
    def load(cls, loadable_state: Union[str, IO]) -> "LoadableMixin":
        ...
        Returns
        -------
        object : LoadableMixin
            An instantiated loadable object.
        """
        if not isinstance(loadable_state, dict):
            loadable_state = torch.load(loadable_state)
        hint = cls if not cls._abstract_loadable else None
...
def make_loadable(klass, save_args: bool = True) -> Type[DynamicLoadableMixin]:
    assert False, "This function should have been replaced in io.__init__"


def load(
    model_state: Union[str, IO],
    hint: Union[type, Iterable[type], None] = None
) -> LoadableMixin:
    """
    Load and build a model/module from a file.
    ...
    hint : [list of] type
        Possible object types, that will be tried in order.
        If `None` (or all hint fail), trust the type saved in the state.
...
def _validate_loadable(module: nn.Module, base: type = LoadableMixin) -> None:
    """
...
def _validate_loadable_all(
    modules: Iterable[nn.Module],
    base: type = LoadableMixin
) -> None:
    """
    ...
    for module in modules:
        _validate_loadable(module, base)
